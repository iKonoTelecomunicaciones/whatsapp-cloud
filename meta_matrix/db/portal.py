...
class Portal:
    ...
    _columns = "ps_id, app_page_id, mxid, relay_user_id"
    ...
    async def update(self) -> None:
        q = """
            UPDATE portal
            SET ps_id=$1, app_page_id= $2, mxid=$3, relay_user_id=$4 WHERE ps_id=$1
        """
        await self.db.execute(q, *self._values)

    @classmethod
    async def get_by_ps_id(cls, ps_id: str) -> Optional["Portal"]:
        q = f"SELECT {cls._columns} FROM portal WHERE ps_id=$1"
