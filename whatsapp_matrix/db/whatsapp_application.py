...
class WhatsappApplication:
    ...
    _columns = "business_id, ws_phone_id, name, admin_user, page_access_token"
    ...
    async def update(
        ...
        q = """
            UPDATE ws_application
            SET name=$1, admin_user=$2, business_id=$3, ws_phone_id=$4 ,page_access_token=$5 WHERE name=$1
        """
        await cls.db.execute(q, name, admin_user, business_id, ws_phone_id, page_access_token)
    ...
    async def get_by_name(cls, name: str) -> Optional["WhatsappApplication"]:
        q = f"SELECT {cls._columns} FROM ws_application WHERE name=$1"
    ...
    async def get_by_admin_user(cls, admin_user: UserID) -> Optional["WhatsappApplication"]:
        q = f"SELECT {cls._columns} FROM ws_application WHERE admin_user=$1"
    ...
    @classmethod
    async def get_all_ws_apps(cls) -> List[WsBusinessID]:
        q = f"SELECT {cls._columns} FROM ws_application WHERE business_id IS NOT NULL"
