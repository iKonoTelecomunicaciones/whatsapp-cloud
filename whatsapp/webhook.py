...
class WhatsappHandler:
    ...
    async def receive(self, request: web.Request) -> None:
        ...
        new_contacts = [
            {"user_id": contact.get("user_id"), "profile": contact.get("profile")}
            for contact in contacts
        ]

        self.log.critical(f">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>DATA1: {data}")


        data["entry"][0]["changes"][0]["value"]["contacts"] = new_contacts

        self.log.critical(f">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>DATA2: {data}")
